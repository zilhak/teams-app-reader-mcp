...
macOS·Windows에서 **Microsoft Teams(신 Teams 2)** 가 로컬에 캐싱해둔 대화/메시지를 읽는 MCP 서버 (Rust).
...
| flavor | 내용 | 쓰이는 곳 |
|---|---|---|
| HTML (macOS `public.html` / Windows `CF_HTML`) | 서식이 살아있는 HTML | Teams 입력창에 Cmd+V / Ctrl+V |
| 평문 (`public.utf8-plain-text` / `CF_UNICODETEXT`) | **HTML 원문(태그 포함)** | `pbpaste` 검증용 |
